...
namespace OSS.Pipeline
{
    ...
    {
        private readonly Func<Task<(TrafficSingleValue tsValue, TResult result)>> _exeFunc;

        /// <inheritdoc />
        public SimpleEffectActivity(Func<Task<(TrafficSingleValue tsValue, TResult result)>> exeFunc, string pipeCode=null)
        {
        ...
        /// <inheritdoc />
        protected override Task<(TrafficSingleValue tsValue, TResult result)> Executing()
        {
            return _exeFunc();
        }
    }
    ...
    public class SimpleEffectActivity<TFuncPara, TResult>: BaseEffectActivity<TFuncPara, TResult>// : BaseStraightPipe<TFuncPara, TResult>
    {
        ...
        /// <inheritdoc />
        public SimpleEffectActivity(Func<TFuncPara,Task<(TrafficSingleValue tsValue, TResult result)>> exeFunc,string pipeCode = null)
        {
            if (!string.IsNullOrEmpty(pipeCode))
            {
                PipeCode = pipeCode;
