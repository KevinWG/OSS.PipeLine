...
namespace OSS.EventTask.Extension
{
    ...
    {
        ///  顺序执行
        internal static async Task<GroupExecuteResp<TTData, TTRes>> Executing_Serial<TTData, TTRes>(this IList<EventTask<TTData, TTRes>> tasks, TTData data)
            where TTData : class 
            //where TTRes : class, new()
        {
            var exeResp = new GroupExecuteResp<TTData, TTRes>
            {
                TaskResults = new Dictionary<EventTask<TTData, TTRes>, EventTaskResp<TTRes>>(tasks.Count)
            };
        ...
        //  顺序任务 回退当前任务之前所有任务
        internal static async Task Executing_SerialRevert<TTData, TTRes>(this IDictionary<EventTask<TTData, TTRes>, EventTaskResp<TTRes>> taskResults, TTData data)
            where TTData : class
            //where TTRes : class, new()
        {
